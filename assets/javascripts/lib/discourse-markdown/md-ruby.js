// markdown-it-ruby 1.1.1 (github.com/lostandfound/markdown-it-ruby, MIT),
// inlined: the pretty-text engine bundles only lib/discourse-markdown/*,
// so this file must be self-contained — no vendor imports, no globals.

function ddmdRuby(state, silent) {
  // Validate rp option
  const rp =
    Array.isArray(state.md.ddmdOptions.rp) &&
  ...
  function parseAndPushTokens(content) {
    const parsed = [];
    state.md.inline.parse(content, state.md, state.env, parsed);
    parsed.forEach((t) => state.tokens.push(t));
  }
...
function markdownitRuby(md, options = {}) {
  // Merge default options with user-provided options
  md.ddmdOptions = Object.assign({ rp: [] }, options);
  ...
  md.inline.ruler.before("text", "ddmd_ruby", ddmdRuby);
}
